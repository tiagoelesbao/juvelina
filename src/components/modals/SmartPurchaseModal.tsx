// src/components/modals/SmartPurchaseModal.tsx
...
// Interface para o tipo de opção com popular opcional
interface PurchaseOptionType {
  title: string;
  price: number;
  originalPrice: number;
  description: string;
  benefits: string[];
  popular?: boolean; // Popular é opcional
}

const SmartPurchaseModal: React.FC<PurchaseModalProps> = ({ 
  isOpen, 
  onClose, 
  variant = 'default',
  personalizedTitle
  ...
  // Verificar se o modal já foi fechado nesta sessão
  useEffect(() => {
    const checkSessionStorage = () => {
      const modalClosed = sessionStorage.getItem('modalClosed');
      if (modalClosed === 'true' && isOpen) {
        // Se o modal foi fechado nesta sessão, não o reabra
        onClose();
      }
    };
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(event.target as Node)) {
        // Se clicar fora do modal principal e não estiver no exit intent, fecha
        if (!showExitIntent) {
          handleCloseModal();
        }
      }
  ...
  // Função para fechar o modal e impedir reabertura
  const handleCloseModal = () => {
    // Definir flag no sessionStorage para impedir reabertura
    sessionStorage.setItem('modalClosed', 'true');
    
    // Fechar o modal
    onClose();
    
    // Também fechar exit intent se estiver aparecendo
    if (showExitIntent) {
  ...
  // Definir opções de compra
  const options: Record<PurchaseOption, PurchaseOptionType> = {
    single: {
      title: 'Experimente',
      price: 149.90,
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {Object.entries(options).map(([key, option]) => (
                    <div
                      ...
                      onClick={() => setSelectedOption(key as PurchaseOption)}
                    >
                      {option.popular && (
                        <motion.div
                      ...
                      <div className="text-center">
                        ...
                        <label htmlFor={`desktop-option-${key}`} className="flex items-center justify-center gap-2 cursor-pointer">
                          <input
                            ...
                            onChange={() => setSelectedOption(key as PurchaseOption)}
...
export default SmartPurchaseModal;